...
;;; Commentary:
;; Registers 9 focused layers into `eldoc-documentation-functions' at
;; explicit depths, using `eldoc-documentation-compose' strategy.
;; All layers return strings directly (synchronous protocol); the
;; compose strategy handles callback invocation.
;;
;; Layers (hook order, most-negative depth first):
...
(defun org-eldoc--get-lang-doc-fns (lang)
  "Retrieve and cache the eldoc functions for Babel LANG."
...
(defun org-eldoc--guard ()
  "Return non-nil if the current buffer is too large for eldoc."
  (and (fboundp 'too-long-file-p) (too-long-file-p)))

;;; Layer 1: Clock (depth -100)

(defun org-eldoc--clock (_callback &rest _args)
  "Show the active org-clock task name."
  (unless (org-eldoc--guard)
...
(defun org-eldoc--babel-delegation (_callback &rest _args)
  "Delegate to the src-block language's native eldoc functions.
Returns a string directly for the compose strategy.
        ...
        (cond
         ...
         ((member lang '("emacs-lisp" "elisp"))
          (when (and (fboundp 'elisp-eldoc-var-docstring)
            ...
            ;; Call each function synchronously; collect first non-nil result.
            (or (elisp-eldoc-var-docstring #'ignore)
                (elisp-eldoc-funcall #'ignore))))
         ;; Other languages: use cached temp-buffer scrape.
         (t
          (let ((doc-fns (org-eldoc--get-lang-doc-fns lang)))
            (when doc-fns
              (condition-case nil
  ...
  (unless (org-eldoc--guard)
    (when (and (org-at-table-p)
      ...
      (let* ((row (org-table-current-dline))
             ...
             (formula (ignore-errors (org-table-get-stored-formulas))))
        (concat
...
Uses breadcrumb's idle-timer cache (O(1) amortized).
`breadcrumb-imenu-crumbs' returns a fully propertized string."
  (unless (org-eldoc--guard)
