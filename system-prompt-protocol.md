...
- **Emacs Version Targeting & Unreleased API Safety:** The configuration targets Emacs 29, 30, and the upcoming/unreleased Emacs 31 (currently in pretest as `emacs-31.0.90`; not yet an official stable release). Its APIs (e.g., `grep-edit-mode`, the `user-lisp/` auto-compiled personal-code directory, `treesit-auto-install-grammar`) are subject to change, renaming, or removal prior to final release. You MUST wrap any Emacs 31-specific features in defensive runtime guards (e.g., `(when (fboundp '...) ...)`, `(when (boundp '...) ...)`, or semantic version checks). Avoid obsolete functions, but NEVER blindly assume an unreleased Emacs 31 function exists without verifying it.
