...
;; (defun ar/benchmark-init-export-detailed (filename)
;;   "Export benchmark-init data from the raw data tree.
;; Works even if benchmark-init has been deactivated."
;;   (interactive "FExport detailed report to: ")

;;   ;; Check if benchmark-init is loaded
;;   (unless (featurep 'benchmark-init)
;;     (user-error "benchmark-init is not loaded. Add (require 'benchmark-init) to early-init.el"))

;;   ;; Access the raw data tree directly
;;   (unless (and (boundp 'benchmark-init/durations-tree)
;;                benchmark-init/durations-tree)
;;     (user-error "No benchmark data available. Ensure benchmark-init was active during startup"))

;;   ;; Flatten the tree to get all entries
;;   (let* ((flat-data (benchmark-init/flatten benchmark-init/durations-tree))
;;          ;; Remove the root node
;;          (entries (cdr flat-data)))

;;     (unless entries
;;       (user-error "No benchmark data collected. Make sure benchmark-init loaded before other packages"))

;;     (let* ((parsed-data
;;             ;; Parse each entry
;;             (cl-loop for entry in entries
;;                      for name = (cdr (assq :name entry))
;;                      for type = (cdr (assq :type entry))
;;                      for duration = (cdr (assq :duration-adj entry))
;;                      for total = (cdr (assq :duration entry))
;;                      when (and name duration)
;;                      collect (list
;;                               :module (format "%s" name)
;;                               :type (format "%s" type)
;;                               :duration duration
;;                               :total total)))

;;            ;; Calculate statistics
;;            (durations (mapcar (lambda (d) (plist-get d :duration)) parsed-data))
;;            (valid-durations (cl-remove-if (lambda (x) (<= x 0)) durations))
;;            (total-time (if valid-durations (apply #'+ valid-durations) 0.0))
;;            (count (length parsed-data))
;;            (avg-time (if (> count 0) (/ total-time (float count)) 0.0))
;;            (median-time (if valid-durations
;;                             (nth (/ (length valid-durations) 2)
;;                                  (sort (copy-sequence valid-durations) #'<))
;;                           0.0))
;;            (min-time (if valid-durations (apply #'min valid-durations) 0.0))
;;            (max-time (if valid-durations (apply #'max valid-durations) 0.0))

;;            ;; Sort by duration descending
;;            (sorted-data (sort (copy-sequence parsed-data)
;;                               (lambda (a b)
...
;;         ;; === HEADER ===
;;         (insert "#+TITLE: Emacs Startup Benchmark Report\n")
;;         (insert (format "#+DATE: %s\n" (format-time-string "%Y-%m-%d %H:%M:%S")))
;;         (insert "#+AUTHOR: Generated by benchmark-init\n")
;;         (insert "#+STARTUP: overview\n")
;;         (insert "#+OPTIONS: toc:2\n\n")

;;         ;; === SUMMARY STATISTICS ===
;;         (insert "* Executive Summary\n\n")
;;         (insert (format "Total startup time: *%.2f ms* (%.3f seconds)\n\n"
;;                         total-time (/ total-time 1000.0)))

;;         (insert "** Statistics\n\n")
;;         (insert "| Metric | Value |\n")
;;         (insert "|--------+-------|\n")
;;         (insert (format "| Total packages/modules | %d |\n" count))
;;         (insert (format "| Total loading time | %.2f ms |\n" total-time))
;;         (insert (format "| Average per module | %.2f ms |\n" avg-time))
;;         (insert (format "| Median loading time | %.2f ms |\n" median-time))
;;         (insert (format "| Fastest module | %.2f ms |\n" min-time))
;;         (insert (format "| Slowest module | %.2f ms |\n\n" max-time))

;;         ;; === TOP 10 SLOWEST ===
;;         (insert "** Top 10 Slowest Modules\n\n")
;;         (insert "These modules account for the majority of startup time:\n\n")
;;         (insert "| Rank | Module | Type | Duration (ms) | % of Total | Cumulative % |\n")
;;         (insert "|------+--------+------+---------------+------------+--------------|\n")

;;         (let ((cumulative 0.0))
;;           (cl-loop for i from 0 below (min 10 (length sorted-data))
;;                    for item = (nth i sorted-data)
;;                    for module = (plist-get item :module)
;;                    for type = (plist-get item :type)
;;                    for duration = (plist-get item :duration)
;;                    for percent = (if (> total-time 0)
;;                                      (* 100.0 (/ duration total-time))
;;                                    0.0)
;;                    do (progn
;;                         (setq cumulative (+ cumulative percent))
;;                         (insert (format "| %d | %s | %s | %.2f | %.1f%% | %.1f%% |\n"
;;                                         (1+ i) module type duration percent cumulative)))))
;;         (insert "\n")

;;         ;; === OPTIMIZATION RECOMMENDATIONS ===
;;         (insert "* Optimization Recommendations\n\n")

;;         (when (> (length sorted-data) 0)
;;           (let* ((top-3 (cl-subseq sorted-data 0 (min 3 (length sorted-data))))
;;                  (slow-threshold 50.0)
;;                  (slow-modules (cl-remove-if-not
;;                                 (lambda (item)
;;                                   (> (plist-get item :duration) slow-threshold))
;;                                 parsed-data)))

;;             (insert "** High-Impact Targets\n\n")
;;             (insert "Focus on these modules for maximum startup improvement:\n\n")
;;             (dolist (item top-3)
;;               (let* ((module (plist-get item :module))
;;                      (duration (plist-get item :duration))
;;                      (percent (* 100.0 (/ duration total-time))))
;;                 (insert (format "- *%s* (%.2f ms, %.1f%% of total)\n"
;;                                 module duration percent))))
;;             (insert "\n")

;;             (insert "** General Recommendations\n\n")
;;             (insert (format "- %d modules take >%d ms. Consider lazy-loading with ~:defer~\n"
;;                             (length slow-modules) (truncate slow-threshold)))
;;             (insert "- Use ~:hook~ to load packages only when specific modes activate\n")
;;             (insert "- Use ~:commands~ to autoload only when functions are called\n")
;;             (insert "- Consider ~:defer 0.1~ for UI packages (themes, modeline)\n")
;;             (insert "- Review if any packages can be replaced with lighter alternatives\n\n")))

;;         ;; === COMPLETE DATA TABLE ===
;;         (insert "* Complete Benchmark Data\n\n")
;;         (insert "All modules sorted by loading time (descending):\n\n")
;;         (insert "| Module | Type | Duration (ms) | Total (ms) | % of Total |\n")
;;         (insert "|--------+------+---------------+------------+------------|\n")
...
;;                 (duration (plist-get item :duration))
;;                 (total (plist-get item :total))
;;                 (percent (if (> total-time 0)
;;                              (* 100.0 (/ (plist-get item :duration) total-time))
;;                            0.0)))
;;             (insert (format "| %s | %s | %.2f | %.2f | %.1f%% |\n"
;;                             module type duration total percent))))
;;         (insert "\n")

;;         ;; === METADATA AND NOTES ===
;;         (insert "* Notes\n\n")
;;         (insert "** Column Definitions\n\n")
;;         (insert "- *Module*: Package or file name\n")
;;         (insert "- *Type*: Either ~require~ (package) or ~load~ (file)\n")
;;         (insert "- *Duration (ms)*: Time spent loading this module only\n")
;;         (insert "- *Total (ms)*: Time including all dependencies\n")
;;         (insert "- *% of Total*: Percentage of total startup time\n\n")

;;         (insert "** Methodology\n\n")
;;         (insert "This report was generated from benchmark-init data collected during Emacs startup.\n")
;;         (insert (format "- Generated: %s\n" (format-time-string "%Y-%m-%d at %H:%M:%S")))
;;         (insert (format "- Emacs version: %s\n" emacs-version))
;;         (insert (format "- Total modules measured: %d\n\n" count))

;;         (insert "** Next Steps\n\n")
;;         (insert "1. Focus on the top 3-5 slowest modules\n")
;;         (insert "2. Add ~:defer t~ to their ~use-package~ declarations\n")
;;         (insert "3. Run benchmark again and compare results\n")
;;         (insert "4. Repeat until startup time is acceptable\n"))

;;       (message "✓ Detailed benchmark report exported to %s" filename)

;;       ;; Offer to open the file
;;       (when (y-or-n-p "Open the exported file? ")
;;         (find-file filename)))))
...
(use-package org
  :straight (org :type git
                 ...
                 :build (:not autoloads)
                 :pin "b7bc0ede67f3e2a477f4d6ad0c46a6d80bca5aea")
  :defer t
  :custom
  ;; Core settings
  (org-modules nil)
...
(use-package breadcrumb
  ...
  ;; Max length settings (values are in characters, not percentages)
  (breadcrumb-project-max-length 0.5)  ;; Proportion of available space
  (breadcrumb-imenu-max-length 0.5)

  ;; Idle time before updating breadcrumbs (in seconds)
  (breadcrumb-idle-time 0.5)

  :config
  ;; Function to determine if breadcrumb should be enabled
  (defun my/breadcrumb-mode-p ()
    "Return nil if breadcrumb should be disabled in current buffer."
    (or (derived-mode-p 'org-mode)
        (derived-mode-p 'text-mode)))

  ;; Hook to disable breadcrumb in specific modes
  (defun my/breadcrumb-setup ()
    "Conditionally enable breadcrumb-local-mode."
    (unless (my/breadcrumb-mode-p)
  ...
  ((prog-mode
    conf-mode-hook
    yaml-mode
    json-mode) . my/breadcrumb-setup)

  ;; Explicitly disable for org and text modes
  ((org-mode text-mode markdown-mode) . (lambda () (breadcrumb-local-mode -1))))
#+end_src
...
(defvar ar/file-templates-alist
  '(;; --- Configuration Files ---
    ...
    ;; --- Dockerfile (Variety) ---
    ;; Detects specific keywords in filename or directory if complex,
    ;; otherwise defaults to standard.
    (dockerfile-mode :trigger "__docker_standard")
    ...
    ;; Differentiate between a library (sourced) and a script (executed) based on extension/name?
    ;; For now, use regex for libs.
    ("/lib.*\\.sh$" :mode sh-mode :trigger "__sh_lib")
    (sh-mode :trigger "__sh_script")

    ;; --- Web / Config (YAML/JSON) ---
    ("/\\.github/workflows/.*\\.yml$" :mode yaml-mode :trigger "__gh_action")
    (yaml-mode :trigger "__yaml_base")
    (yaml-ts-mode :mode yaml-mode :trigger "__yaml_base")
    ("/package\\.json$" :mode json-mode :trigger "__package_json")
    ("/tsconfig\\.json$" :mode json-mode :trigger "__tsconfig")
    (json-mode :trigger "__json_base")
    (json-ts-mode :mode json-mode :trigger "__json_base")

    ;; --- Nix ---
    ("/flake\\.nix$" :mode nix-mode :trigger "__nix_flake")
    ("/shell\\.nix$" :mode nix-mode :trigger "__nix_shell")
    (nix-mode :trigger "__nix_pkg")

    ;; --- Org Mode ---
    ;; Project/Task based files vs Notes
    ("/gtd/.*\\.org$" :mode org-mode :trigger "__org_gtd")
    ("/roam/.*\\.org$" :mode org-mode :trigger "__org_roam")
    ...
    ;; --- Text ---
    (text-mode :trigger "__text"))
  "Alist of file template rules.
Order matters: Regex matches on filenames take precedence over Major Mode matches.

Structure: (PREDICATE . PLIST)
PREDICATE:
  - Regex string: Matches buffer file name.
  - Symbol: Matches major mode (checked via `derived-mode-p`).

PLIST keys:
  :trigger - The yasnippet key to expand.
  :mode    - The specific yasnippet table to search (useful for ts-modes).")
...
(defun ar/file-templates-apply ()
  ...
  (interactive)
  ;; Find the FIRST matching rule (regex priority is handled by list order)
  (when-let* ((rule (cl-find-if #'ar/file-template-p ar/file-templates-alist))
              (plist (cdr rule))
              ;; Determine the Yasnippet table mode
    ...
    (if-let ((template (yas-lookup-snippet trigger table-mode)))
        (progn
          (message "Expanding template: %s" trigger)
          (yas-expand-snippet (yas--template-content template) (point-min) (point-max)))
      ;; Fallback debug message
      (message "File Template: Trigger '%s' not found in mode '%s'" trigger table-mode))))
