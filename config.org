...
;; Load custom file with comprehensive error handling
(when (file-exists-p custom-file)
  (condition-case err
      (load custom-file 'noerror 'nomessage)
    (error
     (message "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
     (message "⚠️  WARNING: Failed to load custom-file!")
     (message "Error: %s" (error-message-string err))
     (message "Location: %s" custom-file)
     (message "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
     (message "")
     (message "To fix this issue:")
     (message "1. Backup: cp %s %s.backup" custom-file custom-file)
     (message "2. Delete: rm %s" custom-file)
     (message "3. Restart Emacs")
     (message "")
     (message "The file will be regenerated when you use M-x customize")
     (message "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")

     ;; Optionally, create a backup automatically
     (let ((backup-file (concat custom-file ".error-backup")))
       (when (not (file-exists-p backup-file))
         (copy-file custom-file backup-file t)
         (message "✓ Automatic backup created: %s" backup-file))))))
#+end_src
...
** Eldoc
*Disabled global which-func for now*
#+begin_src emacs-lisp
...
(use-package org-ibullets
  ...
  :vc (:url "https://github.com/jamescherti/org-ibullets.el.git")
...
(use-package dape
  ...
  ;; Optional transient menu setup - only if transient is loaded
  (with-eval-after-load 'transient
    ...
    ;; Define transient groups
    (transient-define-group ar/toggles-dape-stepping ()
      ["Stepping"
    ...
    (transient-define-group ar/toggles-dape-switch ()
      ["Switch"
    ...
    (transient-define-group ar/toggles-dape-breakpoints ()
      ["Breakpoints"
    ...
    (transient-define-group ar/toggles-dape-debug ()
      ["Debug"
       ("d" "dape" dape)
       ("Q" "quit" dape-quit)
       ("q" "" transient-quit-one :format " ")])

    ;; Define main transient prefix
    (transient-define-prefix ar/toggles-dape ()
      "Dape debugger transient menu"
      [:description ar/dape-title
                    'ar/toggles-dape-stepping
                    'ar/toggles-dape-switch
                    'ar/toggles-dape-breakpoints
                    'ar/toggles-dape-debug])

    ;; Display transient menu on startup
    (add-hook 'dape-on-start-hooks #'ar/toggles-dape)))
#+end_src
...
(use-package persp-mode
  ...
  ;; CRITICAL FIX: Delay auto-resume in daemon mode
  (persp-auto-resume-time (if (daemonp) -1.0 2.0))
  ...
  ;; Don't save window configurations (simpler, more reliable)
  (persp-reset-windows-on-nil-window-conf t)

  ;; Silence warnings
  (persp-auto-save-persps-to-their-file-before-kill nil)

  ;; CRITICAL FIX: Prevent frame initialization errors in daemon mode
  (persp-emacsclient-init-frame-behaviour-override -1)

  :init
  ;; Set the autosave file location
  (setq persp-auto-save-fname (expand-file-name "persp-auto-save" user-emacs-directory))

  ;; CRITICAL FIX: Use window-setup-hook for daemon compatibility
  (add-hook 'window-setup-hook
            (lambda ()
              (require 'persp-mode)
              (persp-mode 1))
            100) ;; Run late to ensure frames are ready

  :config
  (let ((persp-dir (file-name-directory persp-auto-save-fname)))
    (unless (file-directory-p persp-dir)
      (make-directory persp-dir t)))

  (unless (file-exists-p persp-auto-save-fname)
    (with-temp-file persp-auto-save-fname
      (insert ";; Persp-mode auto-save file\n")
      (insert "nil")))
            ...
            (lambda (b)
              (let ((name (buffer-name b)))
                (or
                 ...
                 (string-match-p "^\\*\\(Help\\|info\\|Apropos\\)" name)
                 (string-match-p "^\\*Completions\\*" name)
                 (and (string-prefix-p "*" name)
                      (not (string= name "*Messages*"))
                      (not (string= name "*scratch*")))))))

  ;; Always include scratch and Messages
  (add-hook 'persp-activated-functions
            (lambda (type frame-or-window perspective)
      ...
      (list :name     "Perspective Buffers"
            ...
            :items    (lambda ()
                        (let ((persp (get-current-persp)))
                          (if persp
                              (mapcar #'buffer-name (persp-buffers persp))
                            (consult--buffer-query
                             :sort 'visibility
                             :as #'buffer-name)))))
      "Perspective buffer source for consult-buffer.")
  ...
  ;; --- Safe Crash Prevention ---
  (defun ar/persp-before-save-cleanup ()
    "Clean up child frames and markers before saving perspectives."
    (when (and (fboundp 'posframe-delete-all) (featurep 'posframe))
  ...
  (advice-add 'persp-save-state-to-file :before #'ar/persp-before-save-cleanup)

  ;; Safe auto-save wrapper
  (defun ar/persp-safe-auto-save ()
    "Safely auto-save perspectives."
    (condition-case err
        (when (and persp-mode
                   (not (null (persp-names)))
                   ;; CRITICAL: Don't auto-save in daemon without frames
                   (or (not (daemonp))
                       (> (length (frame-list)) 1)))
          (ar/persp-before-save-cleanup)
          (persp-save-state-to-file))
      (error
       (message "Persp-mode auto-save failed: %s" (error-message-string err)))))

  ;; Auto-save every 5 minutes
  (run-with-idle-timer 300 t #'ar/persp-safe-auto-save)

  ;; --- Helper Functions ---
  (defun ar/persp-switch-or-create (name)
  ...
  (defun ar/persp-kill-other-buffers ()
    "Kill all buffers in current perspective except current one."
    (interactive)
    (let* ((persp (get-current-persp))
           (bufs (when persp (persp-buffers persp)))
           (current (current-buffer)))
      (when (yes-or-no-p (format "Kill %d other buffers in perspective %s? "
                                 (1- (length bufs))
...
;;;###autoload
(defun ar/treemacs-current-project ()
  "Add and display current project exclusively in treemacs."
  (interactive)
  (require 'treemacs)
  (require 'projectile)
  (let ((project-root (or (and (fboundp 'projectile-project-root)
                          ...
                          default-directory)))
    (if (and project-root (not (file-equal-p project-root "~")))
        (progn
...
;;;###autoload
(defun ar/treemacs-toggle ()
  "Initialize or toggle treemacs.
Opens only the current project, removing all other projects."
  (interactive)
  (require 'treemacs)
  (pcase (treemacs-current-visibility)
    (`visible (delete-window (treemacs-get-local-window)))
    (_ (ar/treemacs-current-project))))
...
(defun ar/project-switch-to-file ()
  "Switch to a project and immediately find a file in it.
Includes option to manually choose a directory like project.el."
  (interactive)
  (let* ((choose-dir "... (choose a dir)")
    ...
    (projectile-completing-read
     ...
     :action (lambda (project)
               (let ((project-dir (if (equal project choose-dir)
                                     (read-directory-name "Select directory: "
                                                         default-directory
                                                         nil
                                                         t)
                                   project)))
                 ;; Add the manually selected directory to known projects
                 (when (equal project choose-dir)
                   (projectile-add-known-project project-dir))
                 ;; Switch to the project and find file
                 (projectile-switch-project-by-name project-dir))))))

(defun ar/project-switch-with-menu ()
  "Switch to a project and show the dispatch menu."
  (interactive)
  (let ((projectile-switch-project-action #'projectile-commander))
...
*** consult-projectile buffer switching
#+begin_src emacs-lisp
;; Enhanced
(with-eval-after-load 'consult
  (defun ar/consult-project-buffer ()
    ...
    (if (projectile-project-root)
        (let ((buffers (mapcar #'buffer-name
          ...
          (consult-buffer
           (list (cons (concat "Project Buffers ("
                              (projectile-project-name) ")")
                      buffers))))
      (consult-buffer)))

  ;; Optionally rebind projectile's buffer switch
  (with-eval-after-load 'projectile
    (define-key projectile-mode-map [remap projectile-switch-to-buffer]
      #'ar/consult-project-buffer)))
#+end_src
...
Returns a list of absolute paths to bibliography files, or nil if none found."
  (when-let* ((project-root (projectile-project-root)))
    (directory-files-recursively project-root "\\.bib$")))

(defun ar/setup-project-bibliography ()
  "Create a references.bib file in the project root if it doesn't exist."
  (interactive)
  (if-let* ((project-root (projectile-project-root))
            (bib-file (expand-file-name "references.bib" project-root)))
      (if (file-exists-p bib-file)
          (message "Bibliography already exists: %s" bib-file)
        (with-temp-file bib-file
          (insert "% Bibliography for " (file-name-nondirectory (directory-file-name project-root)) "\n")
          (insert "% Created: " (format-time-string "%Y-%m-%d") "\n\n"))
        (message "Created project bibliography: %s" bib-file))
    (message "Not currently visiting a project.")))
#+end_src
...
(use-package cdlatex
  ...
  ;; FIX: Change CDLaTeX math symbol prefix from backtick to @ or another key
  (cdlatex-math-symbol-prefix ?@)  ;; Use @ instead of `
...
(use-package general
  ...
  (ar/global-leader
    ...
    "TAB TAB" '(ar/persp-switch-or-create :wk "Switch perspective")
    "TAB n" '(persp-add-new :wk "New perspective")
    "TAB d" '(persp-kill :wk "Kill perspective")
    "TAB r" '(persp-rename :wk "Rename perspective")
    "TAB [" '(persp-prev :wk "Previous perspective")
    "TAB ]" '(persp-next :wk "Next perspective")
    "TAB b" '(persp-add-buffer :wk "Add buffer")
    "TAB B" '(persp-remove-buffer :wk "Remove buffer")
    "TAB i" '(persp-import-buffers :wk "Import buffers")
    "TAB k" '(ar/persp-kill-other-buffers :wk "Kill other buffers")
    "TAB s" '(persp-save-state-to-file :wk "Save session")
    "TAB l" '(persp-load-state-from-file :wk "Load session"))
