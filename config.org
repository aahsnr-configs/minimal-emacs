...
(with-eval-after-load 'transient
  ...
  ;; 2. Define the Menu
  (transient-define-prefix ar/vc-menu-transient ()
...
This should point to where you copied Doom's templates folder.")

(defvar ar/file-templates-default-trigger "__"
  "Default yasnippet trigger key for file template rules.
This is used when a rule doesn't specify its own :trigger.")

(defvar ar/file-templates-inhibit nil
  "When non-nil, file template expansion is disabled.
Useful for temporarily disabling the feature.")
...
(defvar ar/file-templates-alist
  '(;; General configuration files
    ...
    ;; Emacs Lisp
    ("/\\.dir-locals\\.el$")
    ("\\.el$" :trigger "__package" :mode emacs-lisp-mode)
    ...
    ;; Lua
    ("/main\\.lua$" :trigger "__main.lua" :mode lua-mode)
    ("/conf\\.lua$" :trigger "__conf.lua" :mode lua-mode)
...
Each element is of the form (PREDICATE . PLIST) where:

PREDICATE is either:
  - A major mode symbol (e.g., `python-mode')
  - A regexp matching the file path (e.g., \"/main\\.py$\")

PLIST is a property list that may contain:
  :mode SYMBOL      - Major mode to get the template from
  :trigger STRING   - Yasnippet key to expand (default: `ar/file-templates-default-trigger')
  :trigger FUNCTION - Function to call for custom insertion
  :project BOOLEAN  - If non-nil, only expand in projects
  :when FUNCTION    - Additional predicate function taking filename
  :ignore BOOLEAN   - If non-nil, skip this file entirely")

(defun ar/file-template-p (rule)
  "Check if RULE applies to the current buffer.
RULE is a cons cell (PREDICATE . PLIST)."
  (let ((pred (car rule))
        (plist (cdr rule)))
    (and
     ...
     ;; Return the rule if all checks pass
     rule)))

(defun ar/file-templates-check ()
  "Check if current buffer should have a template inserted.
Returns non-nil if a template was successfully applied."
  (and
   ;; Not inhibited
   (not ar/file-templates-inhibit)
   ;; Has a file name
   buffer-file-name
   ;; Not read-only
   ...
   ;; Buffer is empty
   (bobp) (eobp)
   ;; Not a special buffer
   (not (string-match-p "^[*[:space:]]" (buffer-name)))
   ;; Not an org-capture buffer
...
Returns non-nil if a template was expanded."
  (interactive)
  (when-let ((rule (cl-find-if #'ar/file-template-p ar/file-templates-alist)))
    (apply #'ar/file-templates--expand rule)))

(cl-defun ar/file-templates--expand (pred &key project mode trigger ignore _when)
  "Expand a file template.
PRED is the matching predicate.
PROJECT, MODE, TRIGGER, IGNORE, and WHEN are from the rule's plist."
      ...
      (unless mode
        (setq mode
              (if (and (symbolp pred) (not (booleanp pred)))
                ...
                major-mode)))
      (unless mode
        (user-error "Couldn't determine mode for file template"))

      ;; Determine the trigger
      (unless trigger
        (setq trigger ar/file-templates-default-trigger))
        ...
        (when (require 'yasnippet nil t)
          ...
          (when yas-minor-mode
            (let ((templates (yas--all-templates (yas--get-snippet-tables mode))))
              (when-let ((template (cl-find trigger templates
...
;; Public API for adding custom templates

(defun ar/file-templates--set (pred plist)
  "Internal function to set or remove a file template rule.
PRED is the predicate, PLIST is the property list.
If PLIST is effectively empty, remove the rule for PRED."
  (if (null (car-safe plist))
      (setq ar/file-templates-alist
            (delq (assoc pred ar/file-templates-alist)
                  ar/file-templates-alist))
    (push `(,pred ,@plist) ar/file-templates-alist)))

(defun ar/set-file-template! (pred &rest plist)
  "Register a file template rule.

PRED can be:
  - A major mode symbol (e.g., `python-mode')
  - A regexp string matching file paths

PLIST can contain:
  :when FUNCTION    - Extra predicate taking filename, must return non-nil
  :trigger STRING   - Yasnippet key to expand
  :trigger FUNCTION - Custom function to insert template
  :mode SYMBOL      - Mode to get snippet from (defaults to PRED if it's a mode)
  :project BOOLEAN  - Only expand in projects if non-nil
  :ignore BOOLEAN   - Skip template expansion if non-nil

Example:
  (ar/set-file-template! 'python-mode
    :trigger \"__init\"
    :mode 'python-mode)"
  (ar/file-templates--set pred plist))

(defun ar/set-file-templates! (&rest templates)
  "Register multiple file template rules at once.

TEMPLATES is a list of (PRED . PLIST) rules.

Example:
  (ar/set-file-templates!
   '(python-mode :trigger \"__init\")
   '(\"/setup\\.py$\" :trigger \"__setup\" :mode python-mode))"
  (dolist (template templates)
    (ar/file-templates--set (car template) (cdr template))))

;; Helper function for use in templates

(defun ar/file-templates-get-short-path ()
  "Get a shortened file path for use in template headers.
Returns a path relative to `user-emacs-directory' if possible,
otherwise an abbreviated path."
  (if buffer-file-name
      (let ((path (file-truename buffer-file-name)))
        (or (when (file-in-directory-p path user-emacs-directory)
              (file-relative-name path user-emacs-directory))
            (abbreviate-file-name path)))
    ""))

;; PERFORMANCE: Register templates directory without loading
;; YASnippet will load snippets just-in-time when modes are activated
(with-eval-after-load 'yasnippet
  (when (file-directory-p ar/file-templates-dir)
    ;; Simply add to search path - YASnippet handles JIT loading automatically
    (add-to-list 'yas-snippet-dirs ar/file-templates-dir t)))

;; Activate file templates on new files
(add-hook 'find-file-hook #'ar/file-templates-check)

;; Provide a way to test which rule would match
(defun ar/file-templates-debug ()
  "Debug which file template rule would apply to the current buffer.
Displays the matching rule or an error if no rule matches."
  (interactive)
  (if-let ((rule (cl-find-if #'ar/file-template-p ar/file-templates-alist)))
      (let ((pred (car rule))
            (plist (cdr rule)))
        (message "Found matching rule: %S\n  Predicate: %S\n  Properties: %S\n  Trigger: %S\n  Mode: %S"
                 rule
                 pred
                 plist
                 (or (plist-get plist :trigger) ar/file-templates-default-trigger)
                 (or (plist-get plist :mode)
                     (if (symbolp pred) pred major-mode))))
    (user-error "No file template rule matches this buffer")))
#+end_src
