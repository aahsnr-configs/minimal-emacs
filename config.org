...
(use-package org
  ...
  ;; ============================================
  ;; CRITICAL: All keywords MUST be unique across sequences
  ;; Personal workflow + Goals + GTD (separate and distinct)
  (org-todo-keywords
   '((sequence "TODO(t)" "NEXT(n)" "PROG(p)" "WAIT(w@/!)" "|" "DONE(d!)" "CANCEL(c@)")
     (sequence "PLAN(P)" "ACTIVE(A)" "PAUSED(x)" "|" "ACHIEVED(a)" "DROPPED(D)")
     ;; GTD-specific sequence (used only in ~/org/gtd/ files)
  ...
  ;; AGENDA SETTINGS
  ;; ============================================
  ;; PERFORMANCE: Only include task files, NOT knowledge notes
  ...
  ;; Performance optimizations
  (org-agenda-inhibit-startup t)           ;; CRITICAL: Don't honor startup options
  (org-agenda-dim-blocked-tasks nil)       ;; Performance: disable dimming
  (org-agenda-use-tag-inheritance nil)     ;; Performance: disable tag inheritance
  (org-agenda-ignore-properties '(effort appt category))  ;; Performance: skip properties
  (org-agenda-span 'day)
  (org-tags-column -80)
  ...
  (defun hot-expand (str &optional mod)
    "Expand org template STR with optional MOD."
    (let (text)
  ...
  (setq org-agenda-custom-commands
        '(;; === DAILY DASHBOARD ===
          ...
          ("w" "Weekly Review Agenda"
           ((todo "INBOX"
                  ((org-agenda-overriding-header "📥 GTD Inbox (Must Process)")
                   (org-agenda-files (list (expand-file-name "gtd/inbox.org" my/org-directory)))))
            (todo "GTD-WAIT"
          ...
          ("p" "Projects Dashboard"
           ((tags "LEVEL=1"
                  ((org-agenda-overriding-header "📋 GTD Projects (Execution)")
                   (org-agenda-files (list (expand-file-name "gtd/projects.org" my/org-directory)))))
            (tags "+ACTIVE"
                  ((org-agenda-overriding-header "🚀 Active Project Context")
