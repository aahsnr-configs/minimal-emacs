...
** Package Management
#+begin_src emacs-lisp
(setq straight-check-at-startup nil)
...
This function is optimized for use with straight.el by ensuring stale
byte-compiled files do not interfere with the reload."
  (interactive)
  (let* ((config-org (expand-file-name "config.org" user-emacs-directory))
    ...
    (when (yes-or-no-p "Tangle and reload configuration? ")
      (message "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
      (message "Tangling configuration from config.org...")

      (condition-case err
          (progn
            ;; Ensure Org Babel is loaded for tangling
            (require 'org)
            (require 'ob-tangle)
      ...
      (when (file-exists-p init-elc)
        (delete-file init-elc)
        (message "✓ Removed stale init.elc"))
      ...
      (let ((elapsed-time (float-time (time-subtract (current-time) start-time))))
        ...
        (message "")
        (message "NOTE: Some changes (removed hooks/packages) require a full restart.")
        (message "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")))))
...
(defun ar/eglot-status ()
  "Show the running Eglot server name and command in the echo area."
  (interactive)
  ;; 1. Safely check if eglot is loaded AND an active server exists
  (if (and (fboundp 'eglot-current-server)
           (eglot-current-server))
      (let* ((server (eglot-current-server))
             (proc (jsonrpc--process server))
             ;; 2. Ensure it's a real process before asking for its command
             (cmd (and (processp proc) (process-command proc)))
             ;; 3. Gracefully handle TCP/Network connections or missing commands
             (cmd-str (cond
                       ((listp cmd) (string-join cmd " "))
                       ((stringp cmd) cmd)
                       (t "Network/External Connection"))))

        (message "LSP: %s | Command: %s"
                 (propertize (jsonrpc-name server) 'face 'font-lock-keyword-face)
                 (propertize cmd-str 'face 'font-lock-string-face)))

    (message "No Eglot server managing this buffer.")))
#+end_src
...
(add-hook 'emacs-startup-hook (lambda () (load custom-file 'noerror)))
#+end_src
...
(use-package visual-fill-column
  ...
  :hook ((org-mode . ar/visual-fill-setup)
         (markdown-mode . ar/visual-fill-setup)
         (text-mode . ar/visual-fill-setup)
         (dired-mode . ar/visual-fill-setup)

         ) ; Optional: Centers dired for a cleaner look
  :custom
  ;; width of the text area (default is fill-column, usually 80)
  (visual-fill-column-width 110)
  ;; Center the text within the window
  (visual-fill-column-center-text t)
  ;; Allow splitting windows without the text getting squashed
  (visual-fill-column-enable-sensible-window-split t)
  :config
  (defun ar/visual-fill-setup ()
    "Setup visual wrap and centering."
    (visual-line-mode 1)
...
(defvar my/org-subdirs
  '("roam"
    ...
    "roam/projects"
    "roam/daily"
    "roam/meetings"  ; required by the meeting capture template
    "gtd"
    "reviews"
    "downloads"
    "noter"
...
;;
;; FIX: The registry no longer hardcodes any project (e.g. ~/physics/).
;; project.el already detects projects dynamically via .git presence —
;; no manual registry entry is needed for basic per-project switching.
;; Use `my/register-project-org-context' below to opt specific projects
;; into a custom org context (separate roam DB, gtd dir, capture templates,
;; etc.). Projects without a registry entry are simply left at global context.
...
PLIST accepts the same keys documented in `my/project-org-contexts':
  ...
  :roam-dir       Custom org-roam directory
  :roam-db        Custom org-roam DB path
...
FIX: No hardcoded project roots. Detection is fully dynamic: project.el
identifies any .git-rooted project; only roots that appear in
...
;; FIX: Removed the hardcoded ~/physics/ bootstrap dolist that created
;; directories and seed files for a specific project. Projects are
;; discovered dynamically via project.el (.git detection). Any
;; project-specific setup (directories, seed files) belongs in the
;; project's own init or via my/register-project-org-context.
...
#+RESULTS:
| diredfl-mode | dired-omit-mode | hl-line-mode | dired-hide-details-mode | my/project-org-maybe-switch | diff-hl-dired-mode | ar/visual-fill-setup |
...
(my/register-project-org-context "~/physics/"
  ...
  '(("P"  "Physics")
    ("Pj" "Physics Journal" entry
     (file+olp+datetree "~/physics/journal.org")
     "* %<%H:%M> %?\n:PROPERTIES:\n:CREATED: %U\n:END:\n"
     :empty-lines 1)
    ...
    ("Pf" "Physics Fleeting" plain
     (file "~/physics/notes/physics-00-index.org")
     "\n* %<%Y-%m-%d %H:%M> %?\n"
...
(use-package org-pomodoro
  :defer t
  :config
  ;; FIX: The last cond clause was `(alert-default-style)' which calls
  ;; alert-default-style as a function. It is a variable, not a function,
  ;; so this would throw "Symbol's function definition is void".
  ;; Fixed to `(t alert-default-style)' — a proper default cond clause.
...
(use-package org-agenda
  ...
  :after org
  :config
  ;; NOTE: org-agenda-files is set authoritatively in org-gtd :config below.
...
#+begin_src emacs-lisp
;; org-edna is listed in org-gtd's Package-Requires, so straight.el installs
;; it automatically. No separate use-package block is needed.
...
(use-package org-gtd
  ...
  ;; FIX: Removed the duplicate assignment that was in org-agenda :config.
...
(use-package org-roam
  ...
  (my/ensure-org-dir "roam/meetings")
  ...
  ;; COMMENT: Knowledge notes must not appear in task views.
...
(defun ar/gtd-to-zettel ()
  "Create a Zettelkasten project note from the current GTD project heading.
Inserts a backlink to the new Zettel note from the GTD entry.

FIX: The original used `gtd-id' (the current GTD task's own ID) as the link
target, producing a self-referential link that pointed back to the GTD task
rather than the newly created Zettel note. Fixed by creating the note with
:immediate-finish t, syncing the DB, then resolving the new node's actual ID."
  (interactive)
  ...
  (let* ((heading    (org-get-heading t t t t))
    ...
    ;; Create the roam project note immediately (skips interactive prompt)
    (org-roam-capture
     :node  (org-roam-node-create :title node-title)
     :props '(:immediate-finish t))

    ;; Sync the DB so the new node becomes queryable
    (org-roam-db-sync)

    (let* ((new-node    (org-roam-node-from-title-or-alias node-title))
           (new-node-id (and new-node (org-roam-node-id new-node))))
      (if (null new-node-id)
          (message "Note created, but ID could not be resolved yet — add backlink manually")
        (save-excursion
...
(defun ar/zettel-to-gtd ()
  "Extract actionable tasks from the current Zettelkasten note to the GTD inbox."
  (interactive)
  ...
  (let* ((note-title (org-roam-node-title (org-roam-node-at-point)))
    ...
    ;; FIX: org-map-entries requires a callable FUNC. The original passed `t'
    ;; (not a function), which would throw (wrong-type-argument functionp t).
    ;; Replaced with a proper lambda.
    (org-map-entries
     (lambda ()
       (let ((heading (org-get-heading t t t t))
...
(defun ar/fleeting-to-permanent ()
  "Open the fleeting notes file for review and conversion to permanent notes."
  (interactive)
...
(defun ar/weekly-review ()
  "Launch a comprehensive weekly review: GTD + Zettelkasten with live stats."
  (interactive)
  (let* ((week-string (format-time-string "%Y-W%U"))
         ...
         ;; FIX: org-map-entries FUNC must be callable. The original passed
         ;; bare `t' as the function argument, which throws a wrong-type-argument
         ;; error. Replaced with a lambda returning t so the list length gives
         ;; the entry count.
         (inbox-count
...
;; Add GTD bridge capture template after org-roam loads
...
Top-level keys:
  ...
  [n] Zettelkasten note → org-roam
  [f] Fleeting thought → roam/ideas/fleeting.org
  [j] Journal (sub-menu)
  [m] Meeting notes
  ...
  (let ((choice (read-char-choice
                 (concat "Capture: "
                         "[t]ask [n]ote [f]leeting "
                         "[j]ournal [m]eeting [b]ook "
                         "[h]abit [g]oal [w]eb [i]nbox [q]uit: ")
                 '(?t ?n ?f ?j ?m ?b ?h ?g ?w ?i ?q))))
    (pcase choice
      (?t (org-gtd-capture))

      (?n (org-roam-capture))

      (?f (find-file (expand-file-name "ideas/fleeting.org"
                                       my/org-roam-directory))
          (goto-char (point-max))
          (unless (bolp) (insert "\n"))
          (insert (format "\n* %s\n\n"
...
(use-package org-super-agenda
  ...
  (setq org-super-agenda-groups
        '((:name "🔴 Overdue"
          ...
          (:name "🚗 @Errands"                                  ;; FIX: Added @errands
  ...
  (setq org-agenda-custom-commands
        '(("d" "Daily Dashboard"
          ...
          ("p" "Projects Dashboard"
           ;; FIX: Removed the second block that pulled from
           ;; (expand-file-name "roam/projects" my/org-roam-directory).
           ;; Roam knowledge files contain no TODO items by design, so that
           ;; block always produced an empty agenda section. Removed.
           ((tags "LEVEL=1"
...
(use-package perspective
  ...
  (persp-prevent-killing-last-buffer-in-perspective t)
...
#+begin_src emacs-lisp
;; Custom functions for project.el integration with perspective
;; These provide seamless project-perspective integration without external packages
...
(with-eval-after-load 'perspective
  ...
  (defun ar/persp-project-switch ()
    "Switch to a project and create/switch to its perspective."
    (interactive)
    (let* ((projects (project-known-project-roots))
           (choice (completing-read "Switch to project: " projects nil t))
           (persp-name (file-name-nondirectory
                       (directory-file-name choice))))
      ;; Switch to or create perspective for this project
      (persp-switch persp-name)
      ;; Set default-directory and find file in the project
  ...
  (defun ar/persp-project-kill ()
    ...
    (interactive)
    (when-let* ((project (project-current))
                (persp-name (ar/persp-get-project-name))
                (buffers-to-kill (project-buffers project)))
      (when (yes-or-no-p
...
(use-package treemacs-perspective
  ...
  :config
  ;; Set treemacs scope to work with perspectives
...
*** iBuffer Integration
#+begin_src emacs-lisp
;; IBuffer integration is built into perspective.el via persp-ibuffer
;; No separate package needed

(with-eval-after-load 'perspective
  (defun ar/persp-ibuffer (arg)
    "Open ibuffer filtered by current perspective.
With prefix argument ARG, show all buffers across all perspectives."
    (interactive "P")
    (persp-ibuffer arg))

  (defun ar/persp-ibuffer-filter-by-perspective ()
    "Set ibuffer filter groups by perspective name."
    (interactive)
  ...
  ;; Optional: Auto-filter ibuffer by perspective
  ;; Uncomment the following to enable automatic perspective filtering in ibuffer
  ;; (add-hook 'ibuffer-hook #'ar/persp-ibuffer-filter-by-perspective)
  )
...
#+begin_src emacs-lisp
;; (with-eval-after-load 'perspective
;;   ;; Helper function to kill all buffers in current perspective except current buffer
;;   (defun ar/persp-kill-other-buffers ()
;;     "Kill all buffers in current perspective except the current buffer.
;; This is useful for cleaning up a perspective workspace."
;;     (interactive)
...
;;   ;; Helper function to list all buffers across all perspectives
;;   (defun ar/persp-list-all-buffers ()
;;     "Display all buffers with their associated perspectives in a temporary buffer.
;; This provides an overview of which buffers belong to which perspectives."
;;     (interactive)
;;     (let ((buffer-persp-alist '()))
;;       ;; Collect all buffers and their perspectives
;;       (dolist (persp-name (persp-names))
;;         ;; Use with-perspective to safely access each perspective's buffers
;;         (with-perspective persp-name
;;           (dolist (buf (persp-current-buffers))
;;             (when (buffer-live-p buf)
;;               (push (cons (buffer-name buf) persp-name) buffer-persp-alist)))))

;;       ;; Display results in a temporary buffer
...
;;   ;; Helper function to switch to buffer from all perspectives
;;   (defun ar/persp-switch-buffer-all ()
;;     "Switch to a buffer from all perspectives (not just current).
;; This allows accessing buffers outside the current perspective without
;; having to switch perspectives first."
;;     (interactive)
;;     ;; persp-switch-to-buffer* with prefix arg shows all buffers
...
** Imenu-Breadcrumbs
*Disable breadcrumbs in org buffers*
#+begin_src emacs-lisp
(defun ar/imenu-breadcrumb-get-path (index-alist point)
  "Recursively traverse INDEX-ALIST to find the path to POINT.
Returns a list of strings (e.g., '(\"Class\" \"Method\"))."
  (let ((path nil))
    (dolist (item index-alist)
      (when (and (consp item) (not path))
        (let ((name (car item))
              (val (cdr item)))
          (cond
           ;; Case 1: Nested list (Class/Module with children)
           ((and (listp val) (consp (car val)))
            ;; We need to determine the start/end of this section.
            ;; In standard Imenu/Eglot, a section usually has a special entry
            ;; or we approximate bounds by the range of its children.
            ;; For safety, we check if the point is within the *range* of this node's children.
            (let* ((positions (mapcar (lambda (x)
                                        (if (markerp (cdr x))
                                            (marker-position (cdr x))
                                          (or (cdr x) 0)))
                                      val))
                   (valid-pos (cl-remove-if-not #'numberp positions))
                   (min-pos (if valid-pos (apply #'min valid-pos) 0))
                   (max-pos (if valid-pos (apply #'max valid-pos) (point-max))))
              (when (and (>= point min-pos) (<= point max-pos))
                (setq path (cons name (ar/imenu-breadcrumb-get-path val point))))))

           ;; Case 2: Leaf node (Method/Function)
           ;; Eglot usually provides markers. We check if point is near this marker.
           ((or (markerp val) (integerp val))
            (let ((pos (if (markerp val) (marker-position val) val)))
              ;; Heuristic: If point is after definition start, it's a candidate.
              ;; A stricter check would require end-positions, which Imenu lacks by default,
              ;; but this works well for 'current function' detection.
              (when (and (>= point pos)
                         (< (- point pos) 5000)) ;; Sanity check: don't match if way far off
                (setq path (list name)))))))))
    path))

(defun ar/eldoc-breadcrumbs (callback &rest _)
  "ElDoc backend to show Imenu breadcrumbs (Parent > Child).
Arguments are CALLBACK and ignored."
  (when (and (derived-mode-p 'prog-mode)
             (bound-and-true-p imenu--index-alist)) ; Only run if Imenu is ready
    (let* ((path (ar/imenu-breadcrumb-get-path imenu--index-alist (point)))
           (breadcrumb-string
            (when path
              (concat
               (if (fboundp 'nerd-icons-codicon)
                   (concat (nerd-icons-codicon "nf-cod-symbol_structure" :face 'shadow) " ")
                 "")
               (mapconcat (lambda (s) (propertize s 'face 'shadow))
                          path
                          (propertize " > " 'face 'shadow))))))
      (when breadcrumb-string
        ;; We pass :thing and :face to play nice with other backends
        (funcall callback breadcrumb-string
                 :thing "Breadcrumb"
                 :face 'shadow)))))

;; Register the backend
(defun ar/setup-eldoc-breadcrumbs ()
  "Add breadcrumbs to ElDoc with low priority."
  ;; Priority 10 (positive) means it runs *after* the main docs (usually 0 or -10),
  ;; effectively appending it to the end of the message in the echo area.
  (add-hook 'eldoc-documentation-functions #'ar/eldoc-breadcrumbs 10 t))

;; Hook into Eglot/Prog-mode
(add-hook 'eglot-managed-mode-hook #'ar/setup-eldoc-breadcrumbs)
#+end_src
...
** Completions: LSP + AUCTeX + Company Integration
#+begin_src emacs-lisp
...
** Bash
#+begin_src emacs-lisp
;; Bash/Shell Programming Configuration
;; Sets up LSP (bash-language-server), formatter (shfmt), linter (shellcheck), and debugger (bash-debug)
;; shfmt, shellcheck and bash-debug built-in
;; LSP: bash-language-server with eglot
;; Configures bash-language-server for sh-mode and bash-ts-mode
...
(use-package plantuml-mode
  ...
  ;; ────────────────────────────────────────────────────────────────────────
  ;; Automatically detect the best available PlantUML execution method
...
(use-package general
  ...
  (ar/global-leader
    ...
    "TAB `"   '(persp-switch-last :wk "Last perspective")
    "TAB r"   '(persp-rename :wk "Rename perspective")
    "TAB k"   '(persp-kill :wk "Kill perspective")
    "TAB K"   '(persp-kill-others :wk "Kill other perspectives")

    ;; Buffer management within perspectives
    "TAB b"   '(persp-switch-to-buffer* :wk "Switch to buffer")
    "TAB B"   '(ar/persp-switch-buffer-all :wk "Switch buffer (all persp)")
    "TAB a"   '(persp-add-buffer :wk "Add buffer")
    "TAB A"   '(persp-set-buffer :wk "Set buffer (remove from others)")
    "TAB i"   '(persp-import :wk "Import buffer from perspective")
    ...
    "TAB s s" '(persp-state-save :wk "Save state")
    "TAB s l" '(persp-state-load :wk "Load/restore state")
