...
(use-package persp-mode
  ...
  (persp-auto-save-opt 2) ;; Save on exit and persp-mode deactivation
  (persp-auto-save-num-of-backups 3)
  (persp-auto-resume-time 2.0) ;; Auto-restore after 2 seconds
  ...
  ;; --- Buffer Filtering ---
  ;; Exclude ephemeral buffers from perspectives
  (add-hook 'persp-common-buffer-filter-functions
            (lambda (b)
              (let ((name (buffer-name b)))
                (or
                 ;; Exclude dired-sidebar buffers
                 (string-prefix-p " *" name)
                 ;; Exclude help/info buffers
                 (string-match-p "^\\*\\(Help\\|info\\|Apropos\\)" name)
                 ;; Exclude completion buffers
                 (string-match-p "^\\*Completions\\*" name)
                 ;; Exclude message buffers unless explicitly Messages
                 (and (string-prefix-p "*" name)
                      (not (string= name "*Messages*"))
                      (not (string= name "*scratch*")))))))

  ;; Always include scratch and Messages in all perspectives
  ...
  ;; --- Consult Integration ---
  (with-eval-after-load 'consult
    ;; Hide the default full buffer list
    (consult-customize consult-source-buffer :hidden t :default nil)

    ;; Define perspective-aware buffer source
  ...
  ;; Safe save wrapper for auto-save timer
  (defun ar/persp-safe-auto-save ()
    "Safely auto-save perspectives."
    (condition-case err
        (when (and persp-mode (not (null (persp-names))))
