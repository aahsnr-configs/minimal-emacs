...
;; Register templates directory and ensure they are loaded
...
(defvar ar/file-templates-alist
  '(;; --- Configuration Files ---
    ...
    ;; Handle both standard and tree-sitter modes
    ...
    ;; --- Text / Misc ---
    ;; Specific file names match first
...
Order matches: The first matching rule is applied.
Structure: (PREDICATE . PLIST)

PREDICATE:
  - Regex string: Matches buffer file name (High Priority).
  - Symbol: Matches major mode (Low Priority).

PLIST keys:
  :trigger - The yasnippet key to expand.
  :mode    - The yasnippet table to search (useful for ts-modes).")

(defun ar/file-template-p (rule)
  "Check if RULE applies to the current buffer.
Returns the rule if it applies, nil otherwise."
  (let ((pred (car rule)))
    (cond
     ;; 1. Check Regex against filename
     ((and (stringp pred) buffer-file-name)
      (string-match-p pred buffer-file-name))
     ;; 2. Check Major Mode derivation
     ((symbolp pred)
      (derived-mode-p pred))
     (t nil))))
...
(defun ar/file-templates-check ()
  ...
  (when (and (not (bound-and-true-p ar/file-templates-inhibit))
             ...
             ;; Ignore temp buffers/popups/archives
             (not (string-match-p "^[*[:space:]]" (buffer-name)))
             (not (string-match-p "\\.gpg$" (buffer-name))))
...
(defun ar/file-templates-apply ()
  ...
  (when-let* ((rule (cl-find-if #'ar/file-template-p ar/file-templates-alist))
              (plist (cdr rule))
              ;; If :mode is not specified, use the current major-mode
              (table-mode (or (plist-get plist :mode) major-mode))
              (trigger (or (plist-get plist :trigger) ar/file-templates-default-trigger)))

    ;; Ensure Yasnippet is active
    (unless (bound-and-true-p yas-minor-mode)
      (yas-minor-mode 1))

    ;; Look up and expand
    (if-let ((template (yas-lookup-snippet trigger table-mode)))
        (yas-expand-snippet (yas--template-content template) (point-min) (point-max))
      (message "File Template: Snippet '%s' not found for table '%s'" trigger table-mode))))
...
(defun ar/latex-compile ()
  ...
  (interactive)
  (if (fboundp 'TeX-command-run-all)
...
(defun my/emacs-lisp-mode/load-prettify-symbols ()
  ...
  (setq prettify-symbols-alist
        `(;; Keywords
          ...
          ("defconst" . ,(nerd-icons-codicon "nf-cod-symbol_constant"))
