              ...
              (lambda (frame)
                (with-selected-frame frame
                  ...
                  (set-frame-parameter frame 'buffer-predicate
                                       #'beframe-buffer-list-p)))
              :append))

  (with-eval-after-load 'consult
    (declare-function consult--buffer-state "consult")

    (defun my-beframe-buffer-list-filtered (&optional frame)
      "Return beframed buffer list for FRAME, excluding internal buffers.
Filters out buffers whose names start with '*' (like *scratch*, *Messages*)
and sorts by visibility (most recently used first, excluding current buffer).
If FRAME is nil, use the current frame."
      (let ((frame-buffers (beframe-buffer-list frame :sort #'beframe-buffer-sort-visibility)))
        (seq-remove (lambda (buf)
                      (string-prefix-p "*" (buffer-name buf)))
                    frame-buffers)))

    (setq consult-buffer-list-function #'my-beframe-buffer-list-filtered)))
#+end_src
...
** LSP
#+begin_src emacs-lisp
;; TODO Modify some settings according to https://andreyor.st/posts/2023-09-09-migrating-from-lsp-mode-to-eglot/
(use-package eglot
  :straight (:type built-in)
  :defer t
  :hook
...
(use-package general
  ...
  ;; Frame operations (SPC TAB)
  (ar/global-leader
    "TAB" '(:ignore t :wk "frame")

    ;; Core Frame Management
    "TAB TAB" '(select-frame-by-name :wk "Select frame")
    "TAB n"   '(make-frame-command :wk "New frame")
    "TAB c"   '(my-beframe-switch-or-create-frame :wk "Switch/Create frame")
    "TAB d"   '(delete-frame :wk "Delete frame")
    "TAB D"   '(delete-other-frames :wk "Delete other frames")
    "TAB k"   '(my-beframe-kill-current-buffer-and-frame :wk "Kill buffer & frame")
    "TAB o"   '(other-frame :wk "Other frame")
    "TAB ["   '((lambda () (interactive) (other-frame -1)) :wk "Previous frame")
    "TAB ]"   '((lambda () (interactive) (other-frame 1)) :wk "Next frame")
    "TAB r"   '(beframe-rename-frame :wk "Rename frame")

    ;; Buffer Operations (Frame-specific)
    "TAB b"   '(beframe-switch-buffer :wk "Switch buffer")
    "TAB m"   '(beframe-buffer-menu :wk "Buffer menu")

    ;; Assume Buffers (Add buffers INTO this frame)
    ;; NESTED SYNTAX: This constructs the full sub-menu atomically, preventing
    ;; "starts with non-prefix key" errors on reload.
    "TAB a"   '(:ignore t :wk "assume (add buffers)"
                "f" '(beframe-assume-frame-buffers :wk "All from frame")
                "s" '(beframe-assume-frame-buffers-selectively :wk "Select from frame")
                "S" '(beframe-assume-buffers-selectively-all-frames :wk "Select from all frames")
                "a" '(beframe-assume-all-buffers-no-prompts :wk "All from everywhere")
                "r" '(beframe-assume-buffers-matching-regexp-all-frames :wk "By regexp"))

    ;; Unassume Buffers (Remove buffers FROM this frame)
    ;; NESTED SYNTAX
    "TAB u"   '(:ignore t :wk "unassume (remove buffers)"
                "f" '(beframe-unassume-frame-buffers :wk "All from frame")
                "s" '(beframe-unassume-current-frame-buffers-selectively :wk "Select from current")
                "a" '(beframe-unassume-all-buffers-no-prompts :wk "All (keep global)")
                "r" '(beframe-unassume-buffers-matching-regexp-all-frames :wk "By regexp"))

    ;; Advanced Operations
    "TAB K"   '(beframe-kill-buffers-matching-regexp :wk "Kill by regexp")

    ;; Transient Interface (Alternative to individual commands)
    "TAB t"   '(beframe-transient :wk "Transient menu"))
