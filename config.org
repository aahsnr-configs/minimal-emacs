...
(use-package dashboard
  ...
  :config
  ;; FORCE dashboard to recalculate startup time
  (defun ar/dashboard-refresh-startup-time ()
    "Force dashboard to show current session's startup time."
    (setq dashboard-init-info
          (format "%d packages loaded in %.3f seconds"
                  (length package-activated-list)
                  (float-time (time-subtract after-init-time before-init-time)))))

  (add-hook 'emacs-startup-hook #'ar/dashboard-refresh-startup-time)
...
(use-package tex
  ...
  :hook ((LaTeX-mode . visual-line-mode)
         ;;(LaTeX-mode . prettify-symbols-mode) ;COMMENT Defined else where
  ...
  ;; Indentation
  ;; Proper indentation for itemize/enumerate/description
...
(use-package tex-fold
  ...
  :straight (:type built-in)
  :after tex
  :commands (TeX-fold-mode TeX-fold-buffer)
  :config
  ...
  (add-hook 'LaTeX-mode-hook #'ar/latex-fold-buffer-delayed)
