...
Remove the obsolete TODO comment and the `add-to-list` calls. The `project-switch-commands` alist is now authoritatively defined in the `project.el` `:custom` block to prevent duplication on config reloads.
...
Permanently excise the commented-out `persp-before-switch-functions` hack. `bufferlo` handles spatial isolation and window management natively without requiring manual side-window teardowns.
