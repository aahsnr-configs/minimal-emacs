...
# Go to Implementation — Emacs 31 Implementation

> **VS Code feature:** Jump to concrete implementations of an interface/abstract method.
...
- **`eglot-header-line-mode`**: After jumping to an implementation, the breadcrumb bar immediately reflects the new enclosing class/struct context, maintaining spatial orientation.
...
# Find All References — Emacs 31 Implementation

> **VS Code feature:** Lists every usage of a symbol across the project in the References panel.
...
| Edit references in place | `xref-change-to-xref-edit-mode` | `e` (in `*xref*` buffer) | **Emacs 31 NEW** — enables writable reference buffer for bulk mutation. |
...
# Document Highlight — Emacs 31 Implementation

> **VS Code feature:** Highlights all occurrences of the symbol under the cursor within the current file.
...
| **Emacs routing**      | `eglot` → `eglot-highlight-symbol-mode` → `eglot-highlight-symbol-face` overlay |
...
| **Overlay Engine**    | `eglot-highlight-symbol-mode` | Built-in minor mode that manages the creation and destruction of highlight overlays based on LSP responses.     |
| **Visual Styling**    | `eglot-highlight-symbol-face` | The customizable face applied to the highlighted ranges, inheriting from `highlight` by default.                |
...
| Action                     | Command                           | Keybinding            | Notes                                                                            |
| -------------------------- | --------------------------------- | --------------------- | -------------------------------------------------------------------------------- |
| Toggle symbol highlighting | `eglot-highlight-symbol-mode`     | `SPC t h` (or custom) | Explicitly toggles the highlight overlay engine.                                 |
...
`eglot` enables document highlighting by default in managed buffers. The configuration below focuses on visual customization and ensuring it plays nicely with the performance tuning already established in the config.
...
(use-package eglot
  :ensure nil
  :custom
  ;; Customize the highlight face to match the Tokyo Night theme.
  ;; Uses a subtle background blend to avoid visual clutter.
  (eglot-highlight-symbol-face '((t (:background "#292e42" :foreground unspecified))))
  :config
  ;; Ensure the highlight mode is active in all eglot-managed buffers.
  ;; (This is default behavior in modern eglot, but explicit is safe).
...
| VS Code behavior                              | Emacs 31 equivalent                                                                                            |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| Auto-highlights symbol on cursor stop         | `eglot-highlight-symbol-mode` triggers via `post-command-hook` with an idle delay.                             |
...
- **`doom-themes`**: The customized `eglot-highlight-symbol-face` inherits the Tokyo Night palette (`#292e42`), ensuring the highlight is visible but recessive enough not to compete with `hl-line-mode` or syntax highlighting.
- **`consult`**: If the user needs to _navigate_ the highlighted occurrences rather than just view them, `M-?` (`xref-find-references`) instantly bridges the visual highlight to a searchable `consult-xref` dropdown.
...
# Document Symbols / Outline View — Emacs 31 Implementation

> **VS Code feature:** Tree view of classes/functions/variables in the current file; also powers the breadcrumb dropdown.
...
| **Status**             | 🟢 `working` · `eglot` + `consult` + `eglot-header-line-mode`                                     |
| **Category**           | Diagnostics & Symbols                                                                             |
| **LSP methods**        | `textDocument/documentSymbol`                                                                     |
| **Emacs routing**      | `eglot` → `imenu` / `consult-eglot-symbols` (minibuffer) + `eglot-header-line-mode` (breadcrumbs) |
...
| **Outline Engine**    | `consult-eglot-symbols`      | Renders a live-preview, fuzzy-filtered outline tree in the minibuffer using `vertico` and `orderless`.   |
| **Breadcrumb Engine** | `eglot-header-line-mode`     | Displays a clickable, hierarchical path (e.g., `file › class › method`) in the header line.              |
...
| Toggle breadcrumbs        | `eglot-header-line-mode`             | `SPC t b`     | Enables the clickable path bar at the top of the buffer.   |
...
```emacs-lisp
;; ==========================================
;; 1. EGLOT HEADER LINE (Breadcrumbs)
;; ==========================================
(use-package eglot
  :ensure nil
  :hook ((prog-mode . eglot-header-line-mode)) ; Enable clickable breadcrumbs globally
  :config
  ;; Optional: Customize the separator for breadcrumbs
  (setq eglot-header-line-separator " › "))
...
(ar/global-leader
  ...
  "t b" '(eglot-header-line-mode :wk "Toggle breadcrumbs"))
```
...
| **Breadcrumb rendering** | Native `header-line-format` via `eglot-header-line-mode`, zero third-party dependencies. | Requires `lsp-mode`'s custom breadcrumb implementation.             |
...
| Top bar shows `file › class › method`   | `eglot-header-line-mode` renders this exact hierarchy in the header line.         |
...
- **Enhanced `eglot-header-line-mode`**: The breadcrumb bar in Emacs 31 is more robust, correctly handling deep nesting and long symbol names by truncating gracefully or allowing horizontal scrolling within the header line.
...
- **`eglot-header-line-mode`**: Provides the persistent, clickable breadcrumb trail at the top of the window, updating dynamically as the cursor moves through different scopes.
...
# Workspace Symbol Search — Emacs 31 Implementation

> **VS Code feature:** Ctrl+T "Go to Symbol in Workspace" — fuzzy search symbols across the whole project.
...
_Note: To explicitly trigger the workspace search without remembering the prefix, you can bind a dedicated wrapper:_
...
# Code Actions (Quick Fixes & Refactorings) — Emacs 31 Implementation

> **VS Code feature:** Lightbulb menu: quick fixes, extract method/variable, add missing import, etc.
...
| Toggle lightbulb indicator | `eglot-code-action-indications`      | —                 | Configured via `setq` to show in `margin`, `nearby`, or both.                     |
...
(use-package eglot
  ...
  ;; - 'nearby: Renders indicator inline near the point
...
# Document Formatting (Whole File) — Emacs 31 Implementation

> **VS Code feature:** "Format Document" command (Shift+Alt+F).
...
- **Eglot's `eglot-format-buffer` Refinements**: If you _must_ use the LSP fallback, Emacs 31's `eglot` has improved error handling for `textDocument/formatting` failures, gracefully falling back to a no-op rather than throwing opaque JSON-RPC errors into the `*Messages*` buffer.

## Integration with Existing Stack

- **`eglot`**: Handles all _semantic_ code actions (like "Organize Imports" or "Extract Method"), while `apheleia` handles the _syntactic_ whole-file formatting. This separation of concerns is the gold standard in modern Emacs configurations.
...
This indicates `apheleia` is _not_ running, and you are accidentally triggering `eglot-format-buffer` or a legacy `indent-region` command. Verify your `SPC c f` binding points to `apheleia-format-buffer`.
...
# Range Formatting (Format Selection) — Emacs 31 Implementation

> **VS Code feature:** "Format Selection" command (Shift+Alt+F).
...
| **Status**             | 🟢 `working` · `eglot` (LSP) OR `emacs-lazy-ruff` (CLI, Python)                            |
| **Category**           | Formatting & Editing                                                                       |
| **LSP methods**        | `textDocument/rangeFormatting`                                                             |
| **Emacs routing**      | `eglot` → `eglot-format` (with active region) OR `emacs-lazy-ruff` → `ruff format --range` |
...
| **CLI Alternative (Python)** | `emacs-lazy-ruff`              | Lightweight integration that invokes the Ruff CLI directly (`ruff format --range`) for marked regions and org src blocks, bypassing LSP entirely . |
...
For Python development, `emacs-lazy-ruff` provides a zero-LSP, blazing-fast alternative that leverages Ruff's native `--range` CLI support .

```emacs-lisp
;; ==========================================
;; 1. EMACS-LAZY-RUFF (Python CLI Range Formatting)
;; ==========================================
(use-package emacs-lazy-ruff
  :ensure t
  :defer t
...
| Consideration           | `emacs-lazy-ruff` / `eglot` (chosen)                                                                    | Legacy `python-format` / Heavy LSP UI                                    |
| ----------------------- | ------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **LSP Coupling**        | `emacs-lazy-ruff` requires **zero** LSP integration, using pure CLI .                                   | Tightly coupled to `lsp-mode` or specific language servers.              |
...
| Respects `.ruff.toml` config          | `emacs-lazy-ruff` automatically inherits the project's Ruff configuration.                   |
...
- **Tree-sitter Region Precision**: When combined with `expreg` or `evil-textobj-tree-sitter`, you can select precise AST nodes (e.g., a single function or class) and format _only_ that structural block, avoiding the fragility of text-based line selection.
- **Ruff CLI `--range` Support**: Modern Ruff versions natively support the `--range` flag (e.g., `--range=10:1-20:1`), allowing the CLI to format specific line ranges without touching the rest of the file . `emacs-lazy-ruff` translates Emacs region bounds into this exact CLI syntax.
...
- **`org-mode`**: `emacs-lazy-ruff` explicitly supports `org-src` blocks, making it the definitive choice for literate programming workflows where Python snippets are embedded in documentation .
...
`emacs-lazy-ruff` requires the `ruff` binary to be in your system's `exec-path`. Verify this with `M-x exec-path` or install it via your OS package manager (e.g., `pacman -S ruff` on Arch Linux).

### Org Src Block Formatting Fails

Ensure the source block is explicitly declared as `python` (e.g., `#+begin_src python`). `emacs-lazy-ruff` uses the block's language identifier to route the formatting command correctly.
...
# On-type Formatting — Emacs 31 Implementation

> **VS Code feature:** Auto-reformats as you type trigger characters like `}` or `;`.
...
- **`apheleia`**: While `electric-*` modes handle _on-type_ structural formatting, `apheleia` handles _on-save_ whole-file formatting. This separation of concerns ensures zero latency while typing, and perfect stylistic compliance upon saving.
...
# Rename Symbol — Emacs 31 Implementation

> **VS Code feature:** F2 — project-wide safe rename of a symbol and all its references.
...
# Folding Ranges — Emacs 31 Implementation

> **VS Code feature:** Gutter arrows to collapse/expand code blocks, functions, imports, regions.
...
- **`treesit-fold` Fringe Priority**: The configuration explicitly sets `treesit-fold-indicators-priority` to `-1`, ensuring folding arrows render _behind_ `diff-hl` and `flymake` indicators, preventing visual clutter in the left margin.
...
# Selection Range (Smart Expand/Shrink) — Emacs 31 Implementation

> **VS Code feature:** Shift+Alt+Right / Shift+Alt+Left grows/shrinks selection by syntactic scope.
...
# Linked Editing Range — Emacs 31 Implementation

> **VS Code feature:** Editing one HTML/JSX tag name automatically updates its matching pair.
...
The LSP `textDocument/linkedEditingRange` capability requires a synchronous network round-trip to the language server on _every keystroke_ while inside a tag. This frequently causes noticeable input latency and main-thread blocking.
...
# Call Hierarchy — Emacs 31 Implementation

> **VS Code feature:** Tree view of what calls a function and what it calls (Shift+Alt+H).
...
| **Emacs 31 synergy**    | `eglot-show-call-hierarchy` is a native command added to Eglot in early 2025 .         | External packages duplicate functionality now present in Emacs core.      |
...
# Type Hierarchy — Emacs 31 Implementation

> **VS Code feature:** Tree view of a type's supertypes and subtypes.
...
# Semantic Tokens (Semantic Highlighting) — Emacs 31 Implementation

> **VS Code feature:** Type-aware syntax coloring beyond what static TextMate grammars can do.
...
- **`treesit` Augmentation**: Emacs 31's `treesit` engine provides a robust, C-level baseline. Eglot's semantic tokens are designed to _augment_ this baseline, applying specific faces (like `font-lock-variable-name-face` with a `readonly` modifier) without stripping the underlying structural tree-sitter highlights.
...
# Inlay Hints — Emacs 31 Implementation

> **VS Code feature:** Inline grey annotations showing inferred types, parameter names, etc.
...
# Inline Values — Emacs 31 Implementation

> **VS Code feature:** Shows variable values inline at end-of-line while paused in the debugger.
...
- **Seamless `eglot` Coexistence**: Because `dape` handles _only_ the Debug Adapter Protocol, it coexists perfectly with `eglot` (which handles the Language Server Protocol). There is no protocol overlap or resource contention, allowing both to run simultaneously in the same buffer without conflict.
...
- **`doom-themes`**: The custom `dape-inline-value-face` inherits the Tokyo Night `shadow` and `#73daca` (teal) accents, maintaining a cohesive, professional IDE aesthetic that clearly distinguages debug values from standard code.
...
2. **Check Variable Scope**: Inline values only appear for variables in the _current_ stack frame's scope. If the cursor is on a line where a variable is not yet defined or is out of scope, the overlay will not render.
...
# Document Links — Emacs 31 Implementation

> **VS Code feature:** Ctrl+click-able URLs, file paths, or import references inside source/comments.
...
| Resolves remote paths over SSH       | Emacs 31 `ffap-prefer-remote-file` ensures Tramp paths are prioritized over local同名 files.  |
...
# Document Color — Emacs 31 Implementation

> **VS Code feature:** Inline color swatches next to CSS-like color values, with a picker.
...
# Diagnostics (Push Model) — Emacs 31 Implementation

> **VS Code feature:** Server pushes errors/warnings; shown as squiggles and in the Problems panel.
...
- **`flymake-show-diagnostics-at-end-of-line 'fancy`**: A game-changer for inline diagnostics. Instead of truncating messages in the echo area or requiring a floating child-frame (`flyover`), Emacs 31 lays out diagnostics _below_ the affected line using Unicode graphics that point back to the exact locus of the error.
...
# Diagnostics (Pull Model) — Emacs 31 Implementation

> **VS Code feature:** Client requests diagnostics on demand (LSP 3.17+), including workspace-wide refresh.
...
# Moniker — Emacs 31 Implementation

> **VS Code feature:** Cross-repository symbol identity, mainly used for large-scale/indexed code navigation.
...
# Workspace File-Operation Hooks — Emacs 31 Implementation

> **VS Code feature:** Automatically fixes up imports/references when files are renamed, moved, or deleted in the Explorer.
...
# Execute Command — Emacs 31 Implementation

> **VS Code feature:** Server-defined custom commands surfaced in the Command Palette (e.g., "Organize Imports", "Restart Server", "Expand Macro").
...
1. **Formatting & Import Sorting**: Handled asynchronously on save by `apheleia` running `ruff-isort` and `ruff`. This is faster and more reliable than asking the LSP server to do it.
2. **Semantic Refactoring**: `eglot-execute-command` (or `eglot-code-actions`) is reserved for deep AST-aware operations that `ruff` cannot perform, such as "Extract to variable", "Convert to f-string", or "Go to Source Definition" (in `basedpyright`/`ty`).
...
> **VS Code feature:** Shows the target inline in an expandable panel without switching editor tabs (Alt+F12 / Shift+Alt+F12).
...
The `peek-xref-references` wrapper is designed to show the _first_ reference inline to maintain the "peek" paradigm. For a comprehensive list of all references, use the standard `xref-find-references` (`M-?` or `SPC c D`), which leverages `consult-xref` for a searchable, multi-candidate dropdown.
...
(use-package breadcrumb
  ...
  :config
  ;; Enable globally. It conservatively activates only in project buffers.
  (breadcrumb-mode 1)
...
- **`consult`**: While `breadcrumb` provides the _visual_ trail, `consult-eglot-symbols` (`SPC c s`) provides the _interactive_ counterpart, allowing users to fuzzy-search the exact same document symbol tree with live buffer previews.
...
| Project diagnostics search | `consult-flymake` (with `C-u`)     | `SPC c E`         | Fuzzy-searches all diagnostics across the entire workspace .                             |
...
(ar/global-leader
  "c" '(:ignore t :wk "code")
  "c e" '(consult-flymake :wk "Search errors (buffer)")
  "c E" '((lambda () (interactive) (consult-flymake t)) :wk "Search errors (project)")
  "c n" '(flymake-goto-next-error :wk "Next error")
  "c p" '(flymake-goto-prev-error :wk "Prev error")
  "c !" '(flymake-start :wk "Force recheck")
  "c P" '(flymake-show-project-diagnostics :wk "Project diagnostics panel"))
...
| VS Code behavior                       | Emacs 31 equivalent                                                                          |
| -------------------------------------- | -------------------------------------------------------------------------------------------- |
| Centralized Problems panel             | `SPC c P` (`flymake-show-project-diagnostics`) or `SPC c E` (`consult-flymake` with prefix). |
| Filter by severity (Errors / Warnings) | Native `/` filter in tabulated-list + `consult-flymake` for fuzzy matching.                  |
| Click error to jump to location        | `RET` on any row in the diagnostics buffer, or fringe/margin clicks.                         |
| "Quick Fix" from panel                 | `SPC c a` (`eglot-code-actions`) at the diagnostic location.                                 |
| Squiggles under erroneous code         | `flymake` renders squiggles via `flymake-error` / `flymake-warning` faces.                   |
...
- **`flymake-show-diagnostics-at-end-of-line 'fancy`**: Instead of truncating messages in the echo area, Emacs 31 lays out diagnostics _below_ the affected line using Unicode graphics that point back to the exact locus of the error.
...
If workspace diagnostic requests cause noticeable lag on massive projects, rely on buffer-local pulls (`SPC c !`) for immediate feedback, and reserve `consult-flymake` (`SPC c E`) for targeted, fuzzy-filtered searches rather than full workspace refreshes.
...
> **VS Code feature:** Matches bracket pairs by color, highlighting nested delimiters with distinct colors.
...
> **VS Code feature:** Miniature file overview on the right edge of the editor with git diff indicators.
...
(add-hook 'minap-mode-hook #'ar/minimap-diff-hl-integration)
...
> **VS Code feature:** Place and edit from multiple cursors simultaneously (arbitrary placement and symbol-based bulk editing).
...
```org
** TODO Iedit
Provides the foundational engine for simultaneous multi-line editing. Highlights all instances of the current symbol or region for synchronized mutation.
...
** TODO Evil Multiedit
Wraps the Iedit engine in a dedicated Evil state for localized multi-cursor refactoring. Maps explicit Vim-mnemonics for cursor accumulation and bulk mutation.
#+begin_src emacs-lisp
...
** TODO Evil Mc
Provides arbitrary and rectangular multi-cursor placement. Creates true, Evil-native fake cursors that flawlessly execute standard Evil motions and operators across all points simultaneously.
#+begin_src emacs-lisp
(use-package evil-mc
  :defer t
  :after evil
...
- **`eglot`**: For _project-wide_ symbol renaming, this multi-cursor setup is intentionally bypassed in favor of `eglot-rename` (`SPC c r`), which uses the LSP server to safely update cross-file references without manual cursor placement.
...
> **VS Code feature:** Visual indicator (lightbulb) in the gutter showing that code actions are available at the cursor.
...
```org
** TODO Quick Fix Lightbulb
Provides a visual indicator in the gutter or via ElDoc when code actions are available at the cursor.
#+begin_src emacs-lisp
(use-package eglot
  :ensure nil
  :custom
...
- **`eglot-code-action-indications` (NEW)**: Emacs 31 introduces native visual indication of available code actions directly within `eglot` . The variable accepts a list of symbols: `margin` (renders in the left margin), `eldoc-hint` (renders via ElDoc), or `mode-line` .
